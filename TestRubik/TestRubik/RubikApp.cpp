...
void RubikApp::Update()
{
	...
	{
		D3DXMatrixTranslation(&T, vertices[i].pos.x/2, vertices[i].pos.y/2, vertices[i].pos.z/2);
		D3DXMatrixTranslation(&T2, -vertices[i].pos.x/2, -vertices[i].pos.y/2, -vertices[i].pos.z/2);
		D3DXVec3Transform(&vT, &vertices[i].pos, &(T*rotX*T2));
...
void RubikApp::Draw()
{
	...
	D3DXMATRIX Ry;
	D3DXMATRIX T;
	D3DXMatrixTranslation(&T, 0.f, -1.f, -15.f);
